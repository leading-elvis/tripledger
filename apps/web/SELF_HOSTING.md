...
目前匯出備份 schema 4，包含各付款人金額、封存狀態、原始及更正後資料、操作人、成員歷史、還款狀態與確認／取消歷程；仍接受 schema 1、2、3 備份。舊單付款人資料及歷史快照會轉為同一人全額付款，原分攤與餘額保持。舊資料沒有保存均分／指定模式，匯入或首次更正採原有分攤金額，不自動重新分配。歷史快照不重複儲存收據，匯入重新配置收據 ID 時不影響歷史。備份不包含平台 subject、有效帳號綁定、邀請密鑰／雜湊、申請者 email 或登入 session。schema 4 備份須使用本版或更新版本還原；不可退回不支援多人付款的舊程式繼續寫入。
