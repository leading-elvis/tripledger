...
內測範圍：每位建立者最多 50 個旅程、每旅程 2–20 位財務旅伴、20 個有效帳號綁定、300 筆支出、200 筆還款、300 筆更正／作廢／旅程異動歷史；旅程 document 上限 1 MiB。邀請與待核准申請各最多 20 筆；累積成員歷史接近上限時停止新邀請與核准，預留移除成員的容量。達上限時拒絕新異動，不刪除舊帳務歷史；封存時預留解除封存的名額與容量。每張收據 1 MiB、旅程收據合計 8 MiB、匯入 JSON 最大 14 MiB。固定財務旅伴與幣別；更正時保留原收據，收據替換另行規劃。目前沒有跨幣別換算、離線記帳、OCR、付款、訂閱或購買權益。帳本記錄款項與還款申報，不會執行轉帳。

目前匯出備份 schema 3，包含封存狀態、原始及更正後資料、操作人、成員歷史、還款狀態與確認／取消歷程；仍接受 schema 1、2 備份。舊資料沒有保存均分／指定模式，匯入或首次更正採原有分攤金額，不自動重新分配。歷史快照不重複儲存收據，匯入重新配置收據 ID 時不影響歷史。備份不包含平台 subject、有效帳號綁定、邀請密鑰／雜湊、申請者 email 或登入 session。schema 3 備份須使用本版或更新版本還原；不可退回會忽略歷史或還款狀態的舊程式繼續寫入。
...
```sh
node --test tests/ledger.test.mjs tests/collaboration.test.mjs tests/navigation.test.mjs
...
多頁面版再加入 3 項路由／登入返回檢查，目前共 19 項測試通過。已在本機驗證首頁、四個帳本頁及加入頁、直接載入與重新整理、瀏覽器前進／返回、未儲存成員設定的離開提示，以及 390px 手機版無橫向溢位。Windows 獨立服務的六個頁面、錯誤網址 404、匿名 API 401 及密碼登入均通過；帳務資料與 schema 3 格式未變更。
