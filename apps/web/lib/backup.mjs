...
export async function createBackup(trip,objects) {
  ...
  return {format:'tripledger-backup',schemaVersion:5,exportedAt:new Date().toISOString(),sourceRevision:trip.revision,trip:clean,files};
}
export async function inspectBackup(input) {
  ensure(input?.format==='tripledger-backup' && [1,2,3,4,5].includes(input.schemaVersion),'不支援此備份格式或版本');
  ...
  if(input.schemaVersion<5&&Array.isArray(input.trip?.history))ensure(!input.trip.history.some(h=>h?.action==='rename-member'),'舊版備份不包含旅伴更名歷史');
