...
function MixedDetails({value,trip}:{value:Expense|ExpenseSnapshot;trip:Trip}) {
  if(value.splitMode!=='mixed'||!value.personalItems?.length)return null;
  const name=(id:string)=>trip.members.find(member=>member.id===id)?.name??'未知旅伴';
  const shared=value.amount-value.personalItems.reduce((sum,item)=>sum+item.amount,0);
  return <div className="mixed-expense-detail"><p className="payment-detail-title">共同平均分攤</p><p>{(value.equalMemberIds??[]).map(name).join('、')} · {formatMoney(shared,trip.currency)}</p><p className="payment-detail-title">個人項目</p><div className="share-detail">{value.personalItems.map((item,index)=><span key={`${index}-${item.memberId}`}><span>{item.name} · {name(item.memberId)}</span><b>{formatMoney(item.amount,trip.currency)}</b></span>)}</div></div>;
}
function Snapshot({value,trip}:{value:ExpenseSnapshot;trip:Trip}) {
  const name=(id:string)=>trip.members.find(m=>m.id===id)?.name??'未知旅伴';
  return <div className="history-snapshot"><b>{value.title} · {formatMoney(value.amount,trip.currency)}</b><p>{value.date} · {value.category} · {value.payments.map(p=>`${name(p.memberId)} ${formatMoney(p.amount,trip.currency)}`).join("、")} 先付{value.voided?' · 已作廢':''}</p><p>{value.shares.map(s=>`${name(s.memberId)} ${formatMoney(s.amount,trip.currency)}`).join('、')}</p><MixedDetails value={value} trip={trip}/></div>;
}
