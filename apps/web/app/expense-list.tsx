...
function Snapshot({value,trip}:{value:ExpenseSnapshot;trip:Trip}) {
  ...
  return <div className="history-snapshot"><b>{value.title} · {formatMoney(value.amount,trip.currency)}</b><p>{value.date} · {value.category} · {name(value.payerId)} 先付{value.voided?' · 已作廢':''}</p><p>{value.shares.map(s=>`${name(s.memberId)} ${formatMoney(s.amount,trip.currency)}`).join('、')}</p></div>;
}
export function ExpenseList({trip,busy,onEdit,onVoid}:{trip:Trip;busy:boolean;onEdit:(expense:Expense)=>void;onVoid:(expense:Expense)=>void}) {
  const [query,setQuery]=useState(''),[category,setCategory]=useState('all'),[status,setStatus]=useState('all'),[from,setFrom]=useState(''),[to,setTo]=useState('');
  ...
  return <><div className="section-head"><h2>旅途中的每一筆</h2><span className="small muted">符合 {expenses.length} 筆</span></div>
    <div className="expense-filters"><label className="field">搜尋支出<input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="支出名稱或付款旅伴"/></label><Choice label="篩選分類" value={category} options={[{value:'all',label:'全部分類'},...Object.keys(categories).map(c=>({value:c,label:c}))]} onChange={setCategory}/><Choice label="紀錄狀態" value={status} options={[{value:'all',label:'全部紀錄'},{value:'active',label:'有效支出'},{value:'voided',label:'已作廢'},{value:'edited',label:'有更正紀錄'}]} onChange={setStatus}/><label className="field">起始日期<input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></label><label className="field">結束日期<input type="date" value={to} onChange={e=>setTo(e.target.value)}/></label><button className="secondary" onClick={reset}>清除篩選</button></div>
    {from&&to&&from>to&&<p className="error" role="alert">起始日期不能晚於結束日期。</p>}
    <p className="small muted">篩選只影響下方列表；總支出與餘額仍依完整帳本計算。</p>
