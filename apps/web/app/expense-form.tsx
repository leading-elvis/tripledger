...
export function ExpenseForm({trip,expense,busy,submit}:{trip:Trip;expense?:Expense;busy:boolean;submit:(data:unknown)=>Promise<void>}) {
  ...
  const save=async(e:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      ...
      await submit({id,operationId,revision,title:form.get('title'),amount:total,payments:paid,category,date:form.get('date'),mode,memberIds:selected,shares,...(mode==='mixed'?{personalItems:personal}:{}),...(!expense?{file:await imagePayload(file)}:{})});
