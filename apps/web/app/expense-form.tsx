...
export function ExpenseForm({trip,expense,busy,submit}:{trip:Trip;expense?:Expense;busy:boolean;submit:(data:unknown)=>Promise<void>}) {
  ...
  const [payments,setPayments]=useState(()=>expense?expense.payments.map(p=>({key:crypto.randomUUID(),memberId:p.memberId,amount:(p.amount/10**digits).toFixed(digits)})):[{key:crypto.randomUUID(),memberId:trip.me.participantId??trip.members[0].id,amount:''}]);
  ...
  const addPayer=()=>{const next=trip.members.find(m=>!payments.some(p=>p.memberId===m.id));if(next)setPayments(old=>[...old,{key:crypto.randomUUID(),memberId:next.id,amount:paymentInputsValid&&totalMinor>paidMinor?((totalMinor-paidMinor)/10**digits).toFixed(digits):''}]);};
