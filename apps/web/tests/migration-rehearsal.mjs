...
try{
  ...
  const report={passed:true,source:'Sites local D1 + R2',destination:'Windows Node.js + SQLite + local receipts (no Sites runtime)',tripId:trip.id,expenses:after.expenses.length,repayments:after.repayments.length,receiptSha256:backup.files[0].sha256,balances:balances(after),restartVerified:true,duplicateImportRejected:true};
