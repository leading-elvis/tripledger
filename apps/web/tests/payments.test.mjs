...
test('real legacy payer fields and history upgrade from schemas 1-3; schema 4 requires every payment snapshot',async()=>{
  ...
  const backup=await createBackup({...t,revision:3},{get:()=>null});assert.equal(backup.schemaVersion,4);
...
test('near-capacity legacy documents remain readable and round-trip through schema 4',async()=>{
  ...
  const backup=await createBackup(trip,{get:()=>null});assert.equal(backup.schemaVersion,4);
...
test('persistent API: legacy database upgrade, payment retry/CAS, receipt restore and restart',async t=>{
  ...
  const backup=(await call(`trips/${old.id}/backup`)).value;assert.equal(backup.schemaVersion,4);assert.ok(backup.trip.expenses.every(e=>e.payments.length===2));
